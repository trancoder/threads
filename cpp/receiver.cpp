...
void Receiver::start(){
    ...
    mPlotThread = std::thread([this](){            
        try {                 
            ...
            //     recvSocket.async_receive_from(boost::asio::buffer(&recvMsg, sizeof(recvMsg)),senderEndpoint,
            //         [this, &recvMsg](const boost::system::error_code& error, std::size_t bytes_recvd) {
            //             if (!error || error == boost::asio::error::message_size) {
            //                 std::cout << "Received: " <<  bytes_recvd << std::endl;
            //                 if (bytes_recvd == sizeof(recvMsg)) {                                
            //                     std::cout << " x =  " <<  recvMsg.x << std::endl;
            //                     std::cout << " y =  " <<  recvMsg.y << std::endl;
            //                     std::cout << " z =  " <<  recvMsg.z << std::endl;
            //                 }
            //                 else {
            //                     std::cerr << "Received unexpected data size: " << bytes_recvd << std::endl;
            //                 }
...
void Receiver::stop() {
    ...
    std::cout << "Joining mPlotThread...." << std::endl;
    mPlotThread.join();
    std::cout << "Bye mPlotThread!!!" << std::endl;
}

void Receiver::startReceiving( boost::asio::ip::udp::socket& recvSocket){
    boost::asio::ip::udp::endpoint senderEndpoint;
    udpData recvMsg;

    recvSocket.async_receive_from(boost::asio::buffer(&recvMsg, sizeof(recvMsg)), senderEndpoint,
        [this, &recvMsg, &recvSocket](const boost::system::error_code& error, std::size_t bytes_recvd) {
            if (!error || error == boost::asio::error::message_size) {
            ...
            startReceiving(recvSocket); // Continue receiving asynchronously
        });
}